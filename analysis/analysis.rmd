...
inputs = "NULL"
for (id in 1:16) {
  path = paste(result_path, "measurements/input", id, ".csv", sep="")
  input = remove_warmup(read.al(path))
...
ecdf_plot("VERIFYING RESULTS", data.frame(
  measurement = sample(filter(measurements, tag == "measurement-lambda20-id1")$service_time, 15000),
  simulation  = sample(filter(simulations, tag == "simulation-lambda20-id1")$service_time, 15000),
  inputs      = sample(inputs$service_time, 15000)
), 0, 1000, 0, 1)

ecdf_plot("VERIFYING SIMULATION", data.frame(
  simulations_lambda100 = sample(filter(simulations, lambda == 20)$service_time, 15000),
  simulations_lambda200  = sample(filter(simulations, lambda == 20)$service_time, 15000),
  inputs      = sample(inputs$service_time, 15000)
), 0, 1000, 0, 1)

ecdf_plot("VERIFYING MEASUREMENTS", data.frame(
  measurements_lambda100 = sample(filter(measurements, lambda == 20)$service_time, 15000),
  measurements_lambda200  = sample(filter(measurements, lambda == 20)$service_time, 15000),
  inputs      = sample(inputs$service_time, 15000)
), 0, 1000, 0, 1)

ecdf_plot("VERIFYING MEASUREMENTS FOR 100 LAMBDA", data.frame(
  measurements_lambda100_id1 = sample(filter(measurements, tag == "measurement-lambda20-id1")$service_time, 15000),
  measurements_lambda100_id2  = sample(filter(measurements, tag == "measurement-lambda20-id2")$service_time, 15000),
  measurements_lambda100_id3  = sample(filter(measurements, tag == "measurement-lambda20-id3")$service_time, 15000)
), 0, 1000, 0, 1)

ecdf_plot("VERIFYING MEASUREMENTS FOR 200 LAMBDA", data.frame(
  measurements_lambda200_id1 = sample(filter(measurements, tag == "measurement-lambda20-id1")$service_time, 15000),
  measurements_lambda200_id2  = sample(filter(measurements, tag == "measurement-lambda20-id2")$service_time, 15000),
  measurements_lambda200_id3  = sample(filter(measurements, tag == "measurement-lambda20-id3")$service_time, 15000)
), 0, 1000, 0, 1)
```
...
boxplot_plot("NO Outliers", df_mean_to_boxplot, df_sim_to_boxplot, df_imp_to_boxplot, c(0, 100))
boxplot_plot("NO Cold Start", df_mean_to_boxplot, df_sim_to_boxplot, df_imp_to_boxplot, c(0, 1000), 16)
...
ks.test(measurement$service_time, simulation$service_time)
wilcox.test(measurement$service_time, simulation$service_time, alt="two.sided")
require(BWStest)
bws_test(measurement$service_time, simulation$service_time)
t.test(measurement$service_time, simulation$service_time)
