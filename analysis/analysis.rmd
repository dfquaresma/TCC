...
NROW(
  filter(
    filter(measurements, service_time < 80), service_time > 60))
  )
)
```
...
library(tidyr)
library(ggplot2)
ecdf_plot <- function(title, data, xinf, xsup, yinf, ysup, linetype=c("dashed", "solid", "dotted"), cols=c(15, 195, 150), img_name=FALSE) {
...
ecdf_plot("VERIFYING RESULTS", data.frame(
  ...
  simulation  = simulations$service_time,
  inputs      = inputs$service_time
), 10, 80, 0, 1, img_name="results-ecdf.png")
